...
public class SingleFingerInputModule : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler {
    ...
    private int countFingersDown = 0;
    ...
    public void OnPointerDown(PointerEventData data) {
        countFingersDown = countFingersDown + 1;

        if (currentSingleFinger == -1 && countFingersDown == 1) {
            currentSingleFinger = data.pointerId;
            if (needsUs != null)
                needsUs.OnSingleFingerDown(data.position);
        }
    }
    ...
    public void OnPointerUp(PointerEventData data) {
        ...
        if (currentSingleFinger == data.pointerId) {
            ...
            if (needsUs != null)
                needsUs.OnSingleFingerUp(data.position);
        }
    }

    public void OnDrag(PointerEventData data) {
        if (currentSingleFinger == data.pointerId && countFingersDown == 1) {
            if (needsUs != null)
                needsUs.OnSingleFingerDrag(data.delta);
        }
    }
